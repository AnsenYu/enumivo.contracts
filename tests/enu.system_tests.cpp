...
BOOST_FIXTURE_TEST_CASE( buysell, enu_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.0000"), get_balance(N(enu.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.0000"), get_balance(N(enu.ramfee)) );
   ...
   transfer( "enumivo", "alice1111111", core_sym::from_string("100000000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, enu_system_tester ) try {
   ...
   const auto init_enu_stake_balance = get_balance( N(enu.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_enu_stake_balance + core_sym::from_string("300.0000"), get_balance( N(enu.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_enu_stake_balance, get_balance( N(enu.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, enu_system_tester ) try {
   ...
   issue( "enumivo", core_sym::from_string("1000.0000"), config::system_account_name );
   issue( "enu.stake", core_sym::from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"), get_balance( "alice1111111" ) );

   //enumivo stakes for alice with transfer flag

   transfer( "enumivo", "bob111111111", core_sym::from_string("1000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, enu_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "enumivo", "alice1111111", core_sym::from_string("2000.0000"), core_sym::from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, enu_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("2.0000"), core_sym::from_string("0.0002")/*"2.0000 ENU", "0.0002 ENU"*/ ) );
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_votepay_share, enu_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_invariant, enu_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_proxy, enu_system_tester, * boost::unit_test::tolerance(1e-5)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_update_order, enu_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_transition, enu_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto* tbl = control->db().find<enumivo::chain::table_id_object, enumivo::chain::by_code_scope_table>(
   ...
   tbl = control->db().find<enumivo::chain::table_id_object, enumivo::chain::by_code_scope_table>(
                  boost::make_tuple( config::system_account_name,
                                     config::system_account_name,
                                     N(producers2) ) );
   BOOST_REQUIRE( !tbl );
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto old_core_from_string = [&]( const std::string& s ) {
      return enumivo::chain::asset::from_string(s + " " + old_contract_core_symbol_name);
   };
   ...
   t.set_abi(  config::system_account_name, contracts::util::system_abi_old().data() );
   {
      ...
      t.abi_ser.set_abi(abi, enu_system_tester::abi_serializer_max_time);
   }
   ...
   auto* tbl = t.control->db().find<enumivo::chain::table_id_object, enumivo::chain::by_code_scope_table>(
                                    boost::make_tuple( config::system_account_name,
                                                       config::system_account_name,
                                                       N(producers2) ) );
   BOOST_REQUIRE( !tbl );
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, enu_system_tester ) try {
   ...
   transfer( "enumivo", "alice1111111", core_sym::from_string("600000000.0000"), "enumivo" );
...
BOOST_FIXTURE_TEST_CASE( buyname, enu_system_tester ) try {
   ...
   transfer( "enumivo", "nofail", core_sym::from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, enu_system_tester ) try {
   ...
   // alice outbids bob on prefb
   {
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.1001"), get_balance(N(enu.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( namebid_pending_winner, enu_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( ram_inflation, enu_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( enuram_ramusage, enu_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(enu.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   //remove row
   base_tester::push_action( N(enu.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
...
BOOST_FIXTURE_TEST_CASE( ram_gift, enu_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "enumivo", "alice1111111", core_sym::from_string("20.0000"), core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( setabi_bios, TESTER ) try {
   ...
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(enu.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(enu.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(enu.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, enu_system_tester ) try {
   set_abi( N(enu.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(enu.token) );
      _abi_hash abi_hash;
